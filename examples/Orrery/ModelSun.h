...
class ModelSun : public ModelBase
{
  public:
    ModelSun(VmaAllocator vmaAllocator, VBBDevice* pDevice, VBBCanvas* pCanv);
    ~ModelSun(void);

    virtual bool initModel(void) override;  // Init/load the model
    virtual bool drawModel(VkCommandBuffer cmdBuffer, glm::mat4 proj, glm::mat4 mv) override;  // Pass in modelview matrix, projection, etc...
